...
const funcNames = new Map([
    ["faceswap", "快速预览仅替换脸部（5-10秒）"],
    ["faceswapV4_S", "精准渲染替换完整头部（1-2分钟）"],
]);


const genderNames = new Map([
    [system.album.personalWoman.id, system.album.personalWoman.name],
    [system.album.personalMan.id, system.album.personalMan.name],
//    [system.album.personalGirl.id, system.album.personalGirl.name],
//    [system.album.personalBoy.id, system.album.personalBoy.name],
]);
...
export default function hairDesign({simRoomBody, image, user, face, config }: { simRoomBody:any, image: Room, user:any, face:Room, config:any }) {
    ...
    const [gender, setGender] = useState(simRoomBody?.params?.gender || system.album.personalWoman.id);
    let title="个人形象设计";
    let num=1;
            ...
            if (!faces || faces.length==0) {
                ...
                if(faces.length>1){
                    ...
                    if(faces[0].gender == 0){ //女性
                        //if(faces[0].age < 12){
                        //    setGender(system.album.personalGirl.id);    
                        //}else{
                            setGender(system.album.personalWoman.id);
                        //}
                    }else{
                        //if(faces[0].age < 12){
                        //    setGender(system.album.personalBoy.id);    
                        //}else{
                            setGender(system.album.personalMan.id);
                        //}
                    }
                ...
                <div className="page-container">
                    ...
                    <div className="page-tab-image-create">  
                        ...
                        <div className="space-y-4 w-full ">
                            <FormLabel number={`${num++}`} label="选择造型种类"/>
                        ...
                        <div className="space-y-3 w-full justify-center">
                            <FormLabel number={`${num++}`} label="选择造型" onCancel={()=>setTargetUrl("")}/>
                            <InputImage src={targetUrl} />
                            <AlbumRoomSelector title="选择您的新造型" albumId={gender}
                                onSelectFile = {(newFile) => setTargetUrl(newFile)}                                    
                                />
                        </div>

                        <Toggle className="flex flex-col items-center mt-4"
                            sideBySide={func==="faceswapV4_S"} leftText="快速预览造型效果" rightText="精确渲染造型效果"
                    ...
                    <ResultView config={config} loading={loading} error={error} restoredImage={restoredImage} restoredId={restoredId} demoRooms={{func:"personalDesign"}} />
